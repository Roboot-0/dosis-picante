...
// ─── CONFIGURACIÓN ───────────────────────────────────────────────────────────
const PAGO_MOVIL = { banco: "Banesco", telefono: "0414-262-4078", cedula: "V-11.741.520", nombre: "DOSIS C.A." };
const WA_BASE = "https://wa.me/584241788803";
// ────────────────────────────────────────────────────────────────────────────

const PRODUCTOS = [
  { id: "microdosis", nombre: "MICRODOSIS", tagline: "El primer contacto",   precio: 7,  imagen: "/images/microdosis.png", color: "#D97706", nivel: 1 },
  { id: "ahumadosis", nombre: "AHUMADOSIS", tagline: "La que convierte",      precio: 7,  imagen: "/images/ahumadosis.png", color: "#EA580C", nivel: 2 },
  { id: "sobredosis", nombre: "SOBREDOSIS", tagline: "La última advertencia", precio: 15, imagen: "/images/sobredosis.png", color: "#DC2626", nivel: 3 },
  { id: "kit",        nombre: "KIT DOSIS",  tagline: "Colección completa",    precio: 25, imagen: "/images/kit.jpeg",       color: "#9CA3AF", nivel: 3 },
];

type Cart = Record<string, number>;
type DeliveryType = "caracas" | "nacional";
type PaymentMethod = "pagomovil" | "efectivo";
interface FormData { nombre: string; telefono: string; direccion: string; ciudad: string; deliveryType: DeliveryType; }
          ...
          return (
            <div key={p.id} className={`flex items-center gap-3 px-4 py-2.5 transition-colors ${qty > 0 ? "bg-rojo/5" : ""}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-crema/30 font-sans">{p.tagline} · <span className="text-crema/50">${p.precio}</span></p>
              </div>
...
function StepDatos({ form, setForm, onNext, onBack }: { form: FormData; setForm: (f: FormData) => void; onNext: () => void; onBack: () => void }) {
  const valid = form.nombre.trim() && form.telefono.trim() && form.direccion.trim() && form.ciudad.trim();
    ...
    <div className="space-y-3">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-px bg-carbon-medio">
          {([
            { value: "caracas" as DeliveryType, label: "Delivery Caracas", sub: "Te lo llevamos." },
            { value: "nacional" as DeliveryType, label: "Envío Nacional", sub: "A cargo del comprador." },
          ]).map((opt) => (
    ...
    <div className="space-y-3">
      ...
      <button onClick={() => setPayment("pagomovil")}
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 pl-6">
          {[["Banco", PAGO_MOVIL.banco], ["Teléfono", PAGO_MOVIL.telefono], ["Cédula/RIF", PAGO_MOVIL.cedula], ["Nombre", PAGO_MOVIL.nombre]].map(([l, v]) => (
            <div key={l}>
...
function StepComprobante({ comprobante, setComprobante, onNext, onBack, submitting }: {
  ...
  const handleFile = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      setComprobante({ base64, mimeType: file.type, nombre: file.name, preview });
    };
...
function CheckoutModal({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = useState<FormData>({ nombre: "", telefono: "", direccion: "", ciudad: "", deliveryType: "caracas" });
  ...
  const submitPedido = useCallback(async () => {
    ...
    const items = PRODUCTOS.filter((p) => (cart[p.id] ?? 0) > 0).map((p) => ({
      nombre: p.nombre, qty: cart[p.id]!, subtotal: cart[p.id]! * p.precio,
    }));
    const montoBS = tasaBCV ? total * tasaBCV : null;
    try {
  ...
  const reset = () => {
    ...
    setForm({ nombre: "", telefono: "", direccion: "", ciudad: "", deliveryType: "caracas" });
    ...
    <motion.div ref={backdropRef}
      ...
      <motion.div
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-carbon-medio">
          <div className="flex items-center gap-2.5">
            <span className="font-bebas text-lg tracking-[0.2em] text-crema">DOSIS</span>
