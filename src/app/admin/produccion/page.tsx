...
export default function ProduccionPage() {
  ...
  const load = async () => {
    setLoading(true);
    const res = await fetch("/api/admin/produccion");
  ...
  const marcarStock = async (b: Batch) => {
    ...
    await fetch("/api/admin/produccion", {
      ...
      body: JSON.stringify({
        ...
        fields: {
          ...
          SKU: b.fields.SKU,
          "Unidades Producidas": b.fields["Unidades Producidas"],
        },
        updateStock: true,
      }),
    });
    setBatches((prev) =>
      prev.map((x) =>
        x.id === b.id ? { ...x, fields: { ...x.fields, "Stock Actualizado": true } } : x
      )
    );
  ...
  const costoCalc = () => {
    const u = parseInt(form["Unidades Producidas"] as string) || 0;
    const t = parseFloat(form["Costo Total USD"] as string) || 0;
    if (u > 0 && t > 0) return `$${(t / u).toFixed(4)} / unidad`;
    return null;
  };
        ...
        <form onSubmit={submit} className="bg-[#292524] border border-[#44403C] rounded-2xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <div>
              ...
              />
              {costoCalc() && (
                <p className="text-[#DC2626] text-xs mt-1">{costoCalc()}</p>
              )}
            </div>
              ...
              <tbody className="divide-y divide-[#44403C]">
                {batches.map((b) => (
                  <tr key={b.id} className="hover:bg-[#3C3835]/40 transition-colors">
                    <td className="px-4 py-3 font-mono text-[#A8A29E] text-xs">
                      {b.fields["Batch ID"] || "—"}
                    </td>
                    ...
                    <td className="px-4 py-3 text-[#A8A29E] whitespace-nowrap">
                      {formatDate(b.fields.Fecha)}
                    </td>
                    <td className="px-4 py-3 text-[#FAFAF9] font-semibold">
                      {(b.fields["Unidades Producidas"] || 0).toLocaleString()}
                    </td>
                    <td className="px-4 py-3 text-[#FAFAF9]">
                      ${(b.fields["Costo Total USD"] || 0).toFixed(2)}
                    </td>
                    <td className="px-4 py-3 text-[#A8A29E] text-xs">
                      {b.fields["Costo por Unidad USD"]
                        ? `$${b.fields["Costo por Unidad USD"].toFixed(4)}`
                        : "—"}
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      {b.fields["Stock Actualizado"] ? (
                        <span className="text-green-400 text-xs font-medium">✓ Actualizado</span>
                      ) : (
                        <button
                          onClick={() => marcarStock(b)}
                          disabled={updatingStock === b.id}
                          className="text-xs px-2 py-1 bg-[#DC2626]/20 text-[#DC2626] rounded-lg hover:bg-[#DC2626]/30 transition-colors disabled:opacity-50"
                        >
                          {updatingStock === b.id ? "..." : "+ Stock"}
                        </button>
                      )}
                    </td>
                  </tr>
