...
export default function GastosPage() {
  ...
  const load = async () => {
    ...
    if (filterSocio) url += `socio=${encodeURIComponent(filterSocio)}`;
  ...
  useEffect(() => { load(); }, [filterMes, filterCat, filterSocio]);
      ...
      {showForm && (
        <form onSubmit={submit} className="bg-[#292524] border border-[#44403C] rounded-2xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            {/* Socio */}
            <div>
              <label className="text-[#57534E] text-xs mb-1 block">Socio</label>
              <select value={form.Socio}
              ...
              <tbody className="divide-y divide-[#44403C]">
                {gastos.map(g => (
                  <tr key={g.id} className="hover:bg-[#3C3835]/40 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {g.fields.Socio ? (
                        <span className="text-xs px-2 py-1 rounded-lg font-medium bg-[#3C3835] text-[#A8A29E]">
                          {g.fields.Socio}
                        </span>
                      ) : <span className="text-[#44403C] text-xs">—</span>}
                    </td>
